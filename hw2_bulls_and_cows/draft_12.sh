...
do
  ...
  then
    ...
    # Calculate statistics: satisfying positions (bulls)
    for (( i=0; i<${#number}; i++ ));
    do
    ...
    # Calculate statistics: not satisfying positions (cows)
    for (( i=0; i<${#number}; i++ ));
    do
      for (( j=0; j<${#number}; j++ ));
      do
        if [ ${number:$i:1} == ${user_input:$j:1} ] && [ $i != $j ]
        then
    ...
    # Show conclusion
    current_result="$step_counter. $number (Коров - $cows_counter, Быков - $bulls_counter)"
